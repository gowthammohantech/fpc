...
function htmlBody(title: string, body: string, link?: string): string {
  ...
  return [
    ...
    '<tr><td style="background:#14697b;padding:20px 28px">' +
      `<span style="${font};color:#ffffff;font-size:15px;font-weight:600">Elixir Finance Ops</span>` +
    ...
    paragraphs,
    link
      ? `<p style="margin:24px 0 0"><a href="${escapeHtml(link)}" style="display:inline-block;background:#14697b;color:#ffffff;text-decoration:none;font-weight:600;font-size:14px;padding:10px 18px;border-radius:999px">Open in Elixir Finance Ops</a></p>`
      : '',
    '</td></tr>',

    `<tr><td style="border-top:1px solid #e2e8f0;padding:16px 28px;${font};color:#64748b;font-size:12px">`,
    'You are receiving this because of your role in Elixir Finance Ops.',
    '</td></tr>',
