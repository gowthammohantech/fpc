 ...
 * The Treasury inbox.
 *
 * `scope=MINE` (the default) is what Treasury means by "waiting on me": open
  ...
  asyncHandler(async (req, res) => {
    ...
    if (q.scope === 'MINE') {
      if (!principal.permissions.includes('finance_request:act')) {
        throw ApiError.forbidden('Only Treasury has a decision queue');
      }
      ...
      throw ApiError.forbidden('Viewing all Treasury requests requires finance_request:read_all');
    }
...
financeRequestRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!request) throw ApiError.notFound('Treasury request');
 ...
 * Treasury question in the thread rather than by email.
 */
financeRequestRouter.post(
  '/:id/remarks',
  requireAnyPermission('finance_request:read', 'finance_request:read_all'),
